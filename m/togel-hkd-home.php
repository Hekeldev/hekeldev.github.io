<ul class="nav nav-tabs grabgtab" style="margin-bottom:0px; font-size:15px; font-weight:bold; margin-top:0px;">

    <li class=" text-center  " style="width:8%; text-align:center;" onClick="location.href='userarea.php?page=statement&head=home'"><a data-toggle="tab" href="#dashboard_wrap" style="padding-left: 0px; padding-right:0px"><span><img src="https://img.pay4d.info/tab-home-w.png" width="25" alt="" style="padding:3px" /><BR>&nbsp;</span></a></li>

    <li class="active" style="width:15%; text-align:center;" onClick="location.href='userarea.php?page=login&head=home'"><a data-toggle="tab" href="#toto_wrap"><span><img src="https://img.pay4d.info/tab-toto-w.png" width="25" alt="" /><br>Togel</span></a>


    </li>
    <li class="" style="width:15%; text-align:center;" onClick="location.href='userarea.php?page=slots&head=home'"><a data-toggle="tab" href="#games_wrap"><span><img src="https://img.pay4d.info/tab-slot-w.png" width="25" alt="" /><br>Slot</span></a>


    </li>


    <li class="" style="width:15%; text-align:center;" onClick="location.href='userarea.php?page=casino&head=home'"><a data-toggle="tab" href="#livegames_wrap"><span><img src="https://img.pay4d.info/tab-livegame-w.png" width="25" alt="" /><br>Casino</span></a>


    </li>
    <li class="" style="width:15%; text-align:center;" onClick="location.href='userarea.php?page=sport&head=home'"><a data-toggle="tab" href="#sport_wrap"><span><img src="https://img.pay4d.info/tab-sport-w.png" width="25" alt="" /><br>Sport</span></a>

        <span style="position:absolute; margin-top:-70px; margin-left:10px;"><img src="https://img.pay4d.info/joy.png" height="16" alt="" /></span>

    </li>

    <li class="" style="width:15%; text-align:center;" onClick="location.href='userarea.php?page=arcade&head=home'"><a data-toggle="tab" href="#fishing_wrap"><span><img src="https://img.pay4d.info/tab-tembakikan-w.png" width="25" alt="" /><br>Arcade</span></a>

    </li>

    <li class="" style="width:15%; text-align:center;" onClick="location.href='userarea.php?page=sabung&head=home'"><a data-toggle="tab" href="#sabung_wrap"><span><img src="https://img.pay4d.info/tab-sabung-w.png" width="25" alt="" /><br>Sabung</span></a>

        <span style="position:absolute; margin-top:-85px; margin-left:-20px;"><img src="https://img.pay4d.info/new2.png" width="40" alt="" /></span>
    </li>
</ul>
<div style="height:10px; background-color:#FFF" class="grabgtabbottom"></div>

<div class="tab-content">




    <div id="toto_wrap" class="tab-pane fade in active">
        <div class="row" style="margin-top:0px; margin-bottom:10px;">
            <div class="col-xs-4" style="padding-right:2px;">

                <a class="btn btn-default btn-block " href="userarea.php?page=login&head=home"><span class="glyphicon glyphicon-calendar"></span>Pasaran</a>
            </div>
            <div class="col-xs-4" style="padding-right:2px; padding-left:0px">

                <a class="btn btn-default btn-block " href="userarea.php?page=keluaran&head=home"><span class="glyphicon glyphicon-bell"></span>Keluaran</a>
            </div>

            <div class="col-xs-4" style="padding-left:0px">
                <a class="btn btn-default btn-block " href="userarea.php?page=bukumimpi&head=home"><span class="glyphicon glyphicon-book"></span>Buku Mimpi</a>
            </div>
        </div>

        <div style="font-weight:bold; margin-bottom:5px; font-size:12px;">
            <a href="userarea.php?page=login&head=home" class="text-warning">⚪ TOGEL</a>
            <span class="glyphicon glyphicon-menu-right"></span>HK SIANG
        </div>
        <div class="well well-sm">
            <strong>Website : </strong><a href="http://www.hkpools1.com" target="_blank">www.hkpools1.com</a><br>
            <strong>Hari Bukaan : </strong>Setiap Hari<br>
            <strong>Hari Libur : </strong>N/A<br>
            <strong>Jam Tutup : </strong>10:30 WIB <strong style="margin-left:20px">Jam Undian : </strong>11:00 WIB<br>
        </div>
        <div style="margin-bottom:10px" class="form-group">
            <select class="form-control ndc" style="background-color:#FD9;width:100%; text-align-last: center; font-weight:bold; height:35px !important">
                <option value="dc">Discount</option>
                <option value="nodc">No Discount</option>
            </select>
        </div>
        <div class="row">
            <div class="col-xs-6" style="padding-right:3px">
                <a class="btn btn-lg btn-warning btn-block" href="?game=qb" role="button" style="font-weight:bold">4D/3D/2D</a>
                <a class="btn btn-lg btn-warning btn-block" href="?game=bbset" role="button" style="font-weight:bold">Bolak Balik</a>
            </div>
            <div class="col-xs-6" style="padding-left:3px">
                <a class="btn btn-lg btn-warning btn-block" href="?game=set" role="button" style="font-weight:bold">4D/3D/2D Set</a>
                <a class="btn btn-lg btn-warning btn-block" href="?game=2dqb" style="margin-bottom:10px;font-weight:bold">Quick 2D</a>
            </div>
        </div>

        <div class="row" style="margin-top:10px">
            <div class="col-xs-6" style="padding-right:3px">
                <a class="btn btn-lg btn-warning btn-block" href="?game=colokbebas" role="button" style="font-weight:bold">Colok Bebas</a>
                <a class="btn btn-lg btn-warning btn-block" href="?game=coloknaga" role="button" style="font-weight:bold">Colok Naga</a>
            </div>
            <div class="col-xs-6" style="padding-left:3px">
                <a class="btn btn-lg btn-warning btn-block" href="?game=colokmacau" role="button" style="font-weight:bold">Colok Macau</a>
                <a class="btn btn-lg btn-warning btn-block" href="?game=colokjitu" style="margin-bottom:10px;font-weight:bold">Colok Jitu</a>
            </div>
        </div>

        <div class="row" style="margin-top:10px">
            <div class="col-xs-6" style="padding-right:3px">
                <a class="btn btn-lg btn-warning btn-block" href="?game=5050umum" role="button" style="font-weight:bold">5050 Umum</a>
                <a class="btn btn-lg btn-warning btn-block" href="?game=5050kombinasi" role="button" style="font-weight:bold">5050 Kombinasi</a>
            </div>
            <div class="col-xs-6" style="padding-left:3px">
                <a class="btn btn-lg btn-warning btn-block" href="?game=5050special" role="button" style="font-weight:bold">5050 Special</a>
                <a class="btn btn-lg btn-warning btn-block" href="?game=kombinasi" style="margin-bottom:10px;font-weight:bold">Kombinasi</a>
            </div>
        </div>

        <div class="row" style="margin-top:10px">
            <div class="col-xs-6" style="padding-right:3px">
                <a class="btn btn-lg btn-warning btn-block" href="?game=dasar" role="button" style="font-weight:bold">Dasar</a>
            </div>
            <div class="col-xs-6" style="padding-left:3px">
                <a class="btn btn-lg btn-warning btn-block" href="?game=shio" role="button" style="font-weight:bold">Shio</a>
            </div>
        </div>


        <a class="btn btn-primary btn-block" href="?home" role="button" style="font-weight:bold; margin-top:20px"><span class="glyphicon glyphicon-menu-left"></span>Kembali</a>


    </div>



    <div id="games_wrap" class="tab-pane fade in ">

    </div>














</div>