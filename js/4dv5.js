...
$(document).ready(function () {
  ...
  //
  $.ajax({
    url: "webdesktop/data.php?status=time",
...
function checkIP() {
  $.ajax({
    url: "desktop/data.php?task=checkIP",
    success: function (result) {
      if (result != "go") {
        quit();
      }
...
function salinnorek() {
  ...
  setTimeout(function () {
    $("#notesalin").fadeOut(500);
  }, 1000);
}

function checkLastLogin() {
  $.ajax({
    url: "desktop/data.php?task=checkLastLogin",
    success: function (result) {
      if (result != "go") {
        quit();
      }
...
function checkIPPro() {
  $.ajax({
    url: "desktop/data.php?task=checkIPPro",
    success: function (result) {
      if (result == "stop") {
        quit();
      }
...
function checkLastLoginPro() {
  $.ajax({
    url: "desktop/data.php?task=checkLastLoginPro",
    success: function (result) {
      if (result == "stop") {
        alert("Sessi anda telah berakhir, silakan login kembali!");
        quit();
      } else if (result == "stoppass") {
        alert("Password anda telah berubah, silakan login kembali!");
        quit();
      } else if (result == "stoplog") {
        alert("Invalid Token, silakan login kembali!");
        quit();
      }
    },
  });
...
function getStatusMaintenance() {
  $.ajax({
    url: "webdesktop/data.php?status=maintenance",
    success: function (result) {
      if (result == "1") {
...
function getMimpi() {
  ...
  $.ajax({
    url: "desktop/data.php?content=mimpi&dimensi=" + dimensi + "&search=" + vsearch,
    success: function (result) {
...
function getBankOnline() {
  $.ajax({
    url: "desktop/data.php?content=bankonline",
    success: function (result) {
...
function getBetHistory(pasaranid, periode, betid, page) {
  ...
  $.ajax({
    url:
      "desktop/data.php?content=detailTransaksi&pasaranid=" +
      pasaranid +
      "&periode=" +
      periode +
      "&betid=" +
...
function getWinHistory(pasaranid, periode, betid, page, win) {
  ...
  $.ajax({
    url:
      "desktop/data.php?content=detailWin&pasaranid=" +
      pasaranid +
      "&periode=" +
      periode +
      "&betid=" +
...
function clickFunction(pasaran) {
  ...
  $(document).on("change", pasaran + " .ndc", function () {
    var ndc = $(this).val();
    var disornodis = "Discount";
    if (ndc == "nodc") disornodis = "No Discount";
    $(pasaran + " .formContainer").html(loadingImg);
    $(pasaran + " .peraturanContainer").hide();
    $.ajax({
      url: "desktop/data.php?task=discount&ndc=" + ndc,
      success: function (result) {
        $(".ndc").each(function () {
          var pasaran2 = $(this).attr("att");
          var pasar2 = pasaran2.substr(1);
          $(this).val(ndc);
          linkGame4D(pasaran2, pasar2, "4dQb", "qb", "4D3D2D");
        });

        alert("Berhasil ubah setting menjadi " + disornodis);
      },
    });
  });
  ...
  $(document).on("click", pasaran + " .confirm", function () {
    ...
    if (userData.credit >= userData.totalBayar - max_lose) {
      ...
      $(pasaran + " .statusBox")
        .removeClass(
          "alert alert-success alert-danger alert-warning text-center glyphicon glyphicon-alert"
        )
            ...
            setTimeout(function () {
              $(pasaran + " .statusBox").fadeOut(500, function () {
                $(pasaran + " .statusBox")
                  ...
                  .html(
                    'Terima kasih, Data berhasil disubmit!<input type="submit" class="btn btn-block btn-success pull-right okbutton" style="margin-top:20px" value="Lanjut"><div style="clear:both"></div>'
                  )
                  .fadeIn(1000);
              });
            }, 1000);
    ...
    if (game == "colokmacau") {
      ...
      if (nomor1 != "" && nomor2 != "") {
        if (nomor1 == nomor2) {
          ...
          hitungForm(pasaran);
        } else if ($(pasaran + " .checkBet").is(":checked") && att != 0) {
          $(pasaran + " .bet" + att).val($(pasaran + " .bet0").val());
          hitungForm(pasaran);
        }
      }
    } else if (game == "colokjitu") {
    ...
    function () {
      ...
      if (game == "4D3D2D") {
        ...
        if (n1 == 1 && n2 == 1 && n3 == 1 && n4 == 1) {
          $(pasaran + " .game" + att).val("4D");
        } else if (n1 == 0 && n2 == 1 && n3 == 1 && n4 == 1) {
          $(pasaran + " .game" + att).val("3D");
        } else if (n1 == 0 && n2 == 0 && n3 == 1 && n4 == 1) {
          $(pasaran + " .game" + att).val("2D");
        } else if (n1 == 1 && n2 == 1 && n3 == 0 && n4 == 0) {
          $(pasaran + " .game" + att).val("2D Depan");
        } else if (n1 == 0 && n2 == 1 && n3 == 1 && n4 == 0) {
          $(pasaran + " .game" + att).val("2D Tengah");
        } else {
          $(pasaran + " .game" + att).val("");
        }

        if (
          $(pasaran + " .checkBet").is(":checked") &&
          $(pasaran + " .game" + att).val() != ""
        ) {
          $(pasaran + " .bet" + att).val($(pasaran + " .bet0").val());
        }
        ...
        if (game == "4D") {
          pot = $(pasaran + " .bet" + att).attr("pot4");
        }
        if (game == "3D") {
          pot = $(pasaran + " .bet" + att).attr("pot3");
        }
        if (game == "2D") {
          pot = $(pasaran + " .bet" + att).attr("pot2");
        }
        if (game == "2D Depan") {
          pot = $(pasaran + " .bet" + att).attr("pot2dd");
        }
        if (game == "2D Tengah") {
          pot = $(pasaran + " .bet" + att).attr("pot2dt");
        }

        if (game != "") potongan = parseInt((pot * bet) / 100);
        ...
        if (nomor1 == "" || nomor2 == "" || nomor3 == "") {
          ...
          $(pasaran + " .bet" + att).val("");
          hitungForm(pasaran);
        } else if (nomor1 != nomor2 && nomor1 != nomor3 && nomor2 != nomor3) {
          var bet = nonribuan($(pasaran + " .bet" + att).val());
          bet = onlyNumber(bet);
      ...
      } else if (game == "5050kombinasi") {
        var bet = nonribuan($(pasaran + " .bet" + att).val());
        bet = onlyNumber(bet);

        var kei = 0;

        if ($(pasaran + " .nomor" + att).val() == "Mono")
          pot = $(pasaran + " .bet" + att).attr("pot1");
        if ($(pasaran + " .nomor" + att).val() == "Stereo")
          pot = $(pasaran + " .bet" + att).attr("pot2");
        if ($(pasaran + " .nomor" + att).val() == "Kembang")
          pot = $(pasaran + " .bet" + att).attr("pot3");
        if ($(pasaran + " .nomor" + att).val() == "Kempis")
          pot = $(pasaran + " .bet" + att).attr("pot4");
        if ($(pasaran + " .nomor" + att).val() == "Kembar")
          pot = $(pasaran + " .bet" + att).attr("pot5");
  ...
  $(document).on("click", pasaran + " .checkAll1", function () {
    ...
    if (this.checked) {
      // check select status
      $(pasaran + " .checkBox1").each(function () {
  ...
  $(document).on("click", pasaran + " .checkAll2", function () {
    ...
    if (this.checked) {
      // check select status
      $(pasaran + " .checkBox2").each(function () {
        //loop through each checkbox
        this.checked = true; //select all checkboxes with class "checkbox1"
      });
    } else {
  ...
  $(document).on("click", pasaran + " .checkAll3", function () {
    ...
    if (this.checked) {
      // check select status
      $(pasaran + " .checkBox3").each(function () {
        //loop through each checkbox
        this.checked = true; //select all checkboxes with class "checkbox1"
      });
    } else {
...
function checkForm(pasaran) {
  ...
  if (task == "bbset_form" || task == "set_form") {
    $(pasaran + " .task").each(function (index) {
      ...
      var totnomor =
        $(pasaran + " .nomor1" + att).val().length +
        $(pasaran + " .nomor2" + att).val().length +
        $(pasaran + " .nomor3" + att).val().length +
        $(pasaran + " .nomor4" + att).val().length;
      if (task == "bbset_form")
        totnomor = totnomor + $(pasaran + " .nomor5" + att).val().length;
...
function clickForm(pasaran, game, userData) {
  $(document).on(
    "click",
    pasaran + " ." + game + "_form .btn-success",
    function () {
      ...
      if (total == "0") {
        ...
        alert("Account Anda sedang tersuspend, hubungi CS kami!");
      } else if (credit - total < -max_lose && max_lose != 0) {
        alert(
          "Total lebih besar dari Maximum Lose Anda ( -" +
            ribuan(max_lose) +
            " )"
        );
      } else if (total > credit && userstatus == "1") {
        alert("Total lebih besar dari Credit Anda ");
      } else {
        var html = "";
        var pasar = pasaran.substr(1);
        $(pasaran + " .dataConfirm").html("");
        ...
        if (game == "4dqb") {
          ...
          $.post(
            ...
            function (data) {
              $(pasaran + " .dataConfirm").html(
                '<div class="well well-sm form-group form-group-sm"><strong>' +
                  title +
              ...
              $(pasaran + " .dataConfirm").append(
                '<table class="table table-striped table-hover"><tr><th>No.</th><th>Nomor</th><th>Game</th><th style="text-align:right">Bet</th><th style="text-align:right">Potongan</th><th style="text-align:right">Bayar</th><th style="text-align:center; width:130px">Status</th></tr>'
              );
          ...
          /*
				...
				$(pasaran+" .dataConfirm tr:last").after("</table>");
				*/
        } else if (game == "colokbebas") {
          $(pasaran + " .cancel").attr("game", game);
          $(pasaran + " .dataConfirm").append(
            '<table class="table table-striped table-hover"><tr><th>No.</th><th>Nomor</th><th>Bet</th><th>Potongan</th><th>Bayar</th></tr>'
          );
          ...
          $(pasaran + " .bet").each(function (index) {
            ...
            if (realbet >= minbet)
              $(pasaran + " .dataConfirm tr:last").after(
                '<tr class="form-group-sm"><td class="text-primary" style="vertical-align:middle">' +
                  index +
                  '</td><td width="200" class="form-inline"><input readonly type="text" class="form-control" style="width:30px;font-weight:bold" value=' +
                  $(pasaran + " .nomor" + att).val() +
                  '></td><td class="text-primary" style="vertical-align:middle">' +
                  bet +
                  '</td><td class="text-primary" style="vertical-align:middle">' +
                  potongan +
                  '</td><td class="text-primary" style="vertical-align:middle">' +
          ...
          $(pasaran + " .dataConfirm tr:last").after(
            '<tr><td></td><td></td><td></td><td class="text-primary" align=right><strong>Total : </strong></td><td><strong>' +
              total +
              "</strong></td></tr>"
          );
          ...
          $(pasaran + " .bet").each(function (index) {
            ...
            if (realbet >= minbet && nomor1 != nomor2 && nomor2 != "")
              $(pasaran + " .dataConfirm tr:last").after(
                '<tr class="form-group-sm"><td class="text-primary" style="vertical-align:middle">' +
                  index +
                  '</td><td width="200" class="form-inline"><input readonly type="text" class="form-control" style="width:30px;font-weight:bold" value=' +
                  $(pasaran + " .nomor" + att).val() +
                  '>  <input readonly type="text" class="form-control" style="width:30px;font-weight:bold" value=' +
                  $(pasaran + " .nomor2" + att).val() +
                  '></td><td class="text-primary" style="vertical-align:middle">' +
                  bet +
                  '</td><td class="text-primary" style="vertical-align:middle">' +
        ...
        if (game == "colokjitu") {
          ...
          $(pasaran + " .bet").each(function (index) {
            ...
            if (realbet >= minbet && nomor != "" && posisi != "")
              $(pasaran + " .dataConfirm tr:last").after(
                '<tr class="form-group-sm"><td class="text-primary" style="vertical-align:middle">' +
                  index +
                  '</td><td width="200" class="form-inline"><input readonly type="text" class="form-control" style="width:30px;font-weight:bold" value=' +
                  nomor +
                  '>  <input readonly type="text" class="form-control" style="width:100px;font-weight:bold" value=' +
                  posisi +
                  '></td><td class="text-primary" style="vertical-align:middle">' +
                  bet +
                  '</td><td class="text-primary" style="vertical-align:middle">' +
          ...
          $(pasaran + " .dataConfirm tr:last").after(
            "<tr><td></td><td></td><td></td><td align=right><strong>Total : </strong></td><td><strong>" +
              total +
              "</strong></td></tr>"
          );
        ...
        if (game == "5050umum") {
          $(pasaran + " .cancel").attr("game", game);
          $(pasaran + " .dataConfirm").append(
            '<table class="table table-striped table-hover"><tr><th>No.</th><th>Nomor</th><th>Bet</th><th>Kei</th><th>Bayar</th></tr>'
          );
          ...
          $(pasaran + " .bet").each(function (index) {
            ...
            if (realbet >= minbet)
              $(pasaran + " .dataConfirm tr:last").after(
                '<tr class="form-group-sm"><td class="text-primary" style="vertical-align:middle">' +
                  index +
                  '</td><td width="200" class="form-inline"><input readonly type="text" class="form-control" style="width:100px;font-weight:bold" value=' +
                  nomor +
                  '></td><td class="text-primary" style="vertical-align:middle">' +
                  bet +
                  '</td><td class="text-primary" style="vertical-align:middle">' +
          ...
          $(pasaran + " .bet").each(function (index) {
            ...
            if (realbet >= minbet)
              $(pasaran + " .dataConfirm tr:last").after(
                '<tr class="form-group-sm"><td class="text-primary" style="vertical-align:middle">' +
                  index +
                  '</td><td width="200" class="form-inline"><input readonly type="text" class="form-control" style="width:90px;font-weight:bold" value=' +
                  tebak +
                  '> <input readonly type="text" class="form-control" style="width:70px;font-weight:bold" value=' +
                  nomor +
                  '></td><td class="text-primary" style="vertical-align:middle">' +
                  bet +
                  '</td><td class="text-primary" style="vertical-align:middle">' +
        ...
        if (game == "5050kombinasi") {
          $(pasaran + " .cancel").attr("game", game);
          $(pasaran + " .dataConfirm").append(
            '<table class="table table-striped table-hover"><tr><th>No.</th><th width=300>Nomor</th><th>Bet</th><th>Kei</th><th>Bayar</th></tr>'
          );
          ...
          $(pasaran + " .bet").each(function (index) {
            ...
            if (realbet >= minbet)
              $(pasaran + " .dataConfirm tr:last").after(
                '<tr class="form-group-sm"><td class="text-primary" style="vertical-align:middle; width:50px">' +
                  index +
                  '</td><td width="200" class="form-inline"><input readonly type="text" class="form-control" style="width:90px;font-weight:bold" value=' +
                  tebak +
                  '> <input readonly type="text" class="form-control" style="width:100px;font-weight:bold" value=' +
            ...
            if (realbet >= minbet)
              $(pasaran + " .dataConfirm tr:last").after(
                '<tr class="form-group-sm"><td class="text-primary" style="vertical-align:middle; width:50px">' +
                  ...
                  '</td><td width="200" class="form-inline"><input readonly type="text" class="form-control" style="width:90px;font-weight:bold" value=' +
                  tebak +
                  '> <input readonly type="text" class="form-control" style="width:80px;font-weight:bold" value=' +
                  nomor +
                  '> <input readonly type="text" class="form-control" style="width:80px;font-weight:bold" value=' +
                  nomor2 +
                  '></td><td class="text-primary" style="vertical-align:middle">' +
          ...
          $(pasaran + " .bet").each(function (index) {
            ...
            if (realbet >= minbet)
              $(pasaran + " .dataConfirm tr:last").after(
                '<tr class="form-group-sm"><td class="text-primary" style="vertical-align:middle">' +
                  index +
                  '</td><td width="200" class="form-inline"><input readonly type="text" class="form-control" style="width:70px;font-weight:bold" value=' +
                  nomor +
                  '></td><td class="text-primary" style="vertical-align:middle">' +
                  bet +
                  '</td><td class="text-primary" style="vertical-align:middle">' +
        ...
        if (game == "shio") {
          $(pasaran + " .cancel").attr("game", game);
          $(pasaran + " .dataConfirm").append(
            '<table class="table table-striped table-hover"><tr><th>No.</th><th width=200>Nomor</th><th>Bet</th><th>Discount</th><th>Bayar</th></tr>'
          );
          ...
          $(pasaran + " .bet").each(function (index) {
            ...
            if (realbet >= minbet)
              $(pasaran + " .dataConfirm tr:last").after(
                '<tr class="form-group-sm"><td class="text-primary" style="vertical-align:middle; width:50px">' +
                  index +
                  '</td><td width="150" class="form-inline"><input readonly type="text" class="form-control" style="width:120px;font-weight:bold" value=' +
                  nomor +
                  '></td><td class="text-primary" style="vertical-align:middle">' +
                  bet +
                  '</td><td class="text-primary" style="vertical-align:middle">' +
...
function linkGame4D(pasaran, pasar, game, form, aturan) {
  ...
  $(pasaran + " .formContainer").html(loadingImg);
  $.ajax({
    url: "form.php?form=" + form + "&p=" + pasar,
    success: function (result) {
      //alert(pasaran+pasar);
...
function linkGame(pasaran, pasar, game) {
  ...
  $(pasaran + " .formContainer").html(loadingImg);
  $.ajax({
    url: "form.php?form=" + game.toLowerCase() + "&p=" + pasar,
    success: function (result) {
      $(pasaran + " .formContainer").html(result);
      $(pasaran + " .formContainer").fadeTo(500, 1);
      getStatus();
...
function hitungForm(pasaran) {
  ...
  $(pasaran + " .bet").each(function (index) {
    ...
    } else if (game == "5050umum" || game == "5050special" || game == "dasar") {
      var bet = nonribuan($(pasaran + " .bet" + att).val());
      bet = onlyNumber(bet);
...
function getPeraturan(pasaran, game) {
  ...
  $.ajax({
    url: "desktop/data.php?content=peraturan&game=" + game,
    success: function (result) {
      //alert(pasaran+pasar);
      ...
      if (result != "") {
        ...
        var pasar = pasaran.substr(1);
        $.ajax({
          url: "desktop/data.php?content=hadiah&pasaran=" + pasar + "&game=" + game,
          success: function (result) {
            //alert(result);
...
function getStatus() {
  $.getJSON("desktop/data.php?status=pasaran", function (data) {
    for (i = 0; i < data.length; i++) {
      var nama = data[i].name.toLowerCase().replace(/\s+/g, "");

      if (data[i].status == 1) {
        $("#p_" + nama)
          .removeClass("glyphicon-minus-sign text-danger text-warning")
          .addClass("glyphicon-ok-sign text-success");
        $("#" + nama + "_status").html(
          '<div class="well well-sm" style="margin-bottom:4px"><h5>' +
            data[i].name +
            " " +
            data[i].nameid +
            "-" +
        ...
        $("#p_" + nama)
          .removeClass("glyphicon-ok-sign  text-success text-warning")
          .addClass("glyphicon-minus-sign text-danger");
        $("#" + nama + "_statusdown").html(
          '<div class="alert alert-warning">Maaf, Pasaran sudah tutup, Mohon kembali lain waktu...</div>'
        );
        $("#" + nama).fadeOut(500);
      } else if (data[i].status == 2) {
        $("#p_" + nama)
          .removeClass("glyphicon-ok-sign  text-success")
          .addClass("glyphicon-minus-sign text-warning");
        $("#" + nama + "_statusdown").html(
          '<div class="alert alert-warning">Maaf, Account anda sedang di suspend, Mohon hubungi Customer Service...</div>'
        );
        $("#" + nama).fadeOut(500);
        ...
        $("#p_" + nama)
          ...
          .addClass("glyphicon-minus-sign text-warning");
        $("#" + nama + "_statusdown").html(
          '<div class="alert alert-warning">Maaf, Account anda sedang di banned, Mohon hubungi Customer Service...</div>'
        );
        $("#" + nama).fadeOut(500);
      }
      ...
      // Nama Pasaran
      if (
        data[i].name.substring(data[i].name.length - 6, data[i].name.length) ==
        "NODISC"
      ) {
        $(".nama_pasaran_" + nama).html(
          data[i].name.substring(0, data[i].name.length - 6) +
            '<img src="' +
            data[i].img +
...
function getWinBet() {
  $.getJSON("desktop/data.php?status=winBet", function (data) {
    if (data.length > 0) {
      $("#winBet").html("");
...
function getDepoWD() {
  $.getJSON("desktop/data.php?status=depoWD", function (data) {
    if (data.length > 0) {
      $("#depoWD").html("");
      for (i = 0; i < data.length; i++) {
        $("#depoWD").append(
...
function getReferal(t, s) {
  ...
  $.ajax({
    url: "desktop/data.php?content=referal&sort=" + s + "&page=" + t,
    success: function (result) {
...
function getTabKeluaran() {
  ...
  $.ajax({
    url: "desktop/data.php?content=tabKeluaran",
    success: function (result) {
...
function getInbox(i) {
  $.ajax({
    url: "memo.php?content=inbox&page=" + i,
    success: function (result) {
...
function getSent(i) {
  $.ajax({
    url: "memo.php?content=sent&page=" + i,
    success: function (result) {
...
function readMemo(task, page, i) {
  $.ajax({
    url: "data2.php?read=memo&page=" + page + "&task=" + task + "&id=" + i,
    success: function (result) {
...
function getMemoTotal() {
  $.ajax({
    url: "data2.php?content=memoBadge",
    success: function (result) {
...
function getKeluaran(pid, i) {
  if (pid == "") {
    ...
    $.ajax({
      url: "desktop/data.php?content=keluaran&pid=" + pid + "&page=" + i,
      success: function (result) {
...
function getCredit() {
  $.ajax({
    url: "desktop/data.php",
    success: function (result) {
		$(".userCredit").html((result));
		$(".credit").val(result);
...
function getSetting() {
  $.getJSON("desktop/credit.php", function (data) {
...
function getBroadcast() {
  $.ajax({
    url: "desktop/data.php?content=broadcast",
    success: function (result) {
...
function init() {
        ...
        if (deposit ) {
          if (note == "nb" && pengirim == "") {
            ...
            $("#deposit_form").fadeOut(500, function () {
              ...
              $.post(
                "../admin/functions/actions.php?aksi=top_up",
                    ...
                    setTimeout(function () {
                      $("#statusDeposit").fadeOut(500, function () {
                        $("#statusDeposit")
                          ...
                          .addClass("alert alert-danger text-center")
                          .text("Maaf Sedang ada Pending Deposit")
                          .fadeIn(1000);
                      });
                    }, 2000);
                  } else if (data == "noteempty") {
                    setTimeout(function () {
                      $("#statusDeposit").fadeOut(500, function () {
                        $("#statusDeposit")
                          .removeClass(
                          ...
                          .addClass("alert alert-danger text-center")
                          .text("Nomor Pengirim / Kode SN harus diisi")
                          .fadeIn(1000);
                      });
                    }, 2000);
                  } else if (data == "error") {
                    setTimeout(function () {
                      $("#statusDeposit").fadeOut(500, function () {
                        $("#statusDeposit")
                          ...
                          .addClass("alert alert-danger text-center")
                          .text("Error! Silakan ulang kembali")
                          .fadeIn(1000);
                      });
                    }, 2000);
                  }
			...
			alert("Maaf, Deposit minimal Rp" + ribuan (10000));
		  }
      } else {
        alert("Error: Tidak ada Rekening Tujuan, Mohon hubungi CS kami!");
      }
    }
  });

  $(document).on("click", "#konfirmasi_deposit_", function () {
      ...
      if (rektujuan != null && rekasal != null) {
        if (deposit ) {
          if (note == "nb" && pengirim == "") {
            ...
            $("#deposit_form2").fadeOut(500, function () {
              $("#statusDeposit2")
                .removeClass(
                  "alert alert-success alert-danger alert-warning text-center glyphicon glyphicon-alert"
                )
              ...
              $.post(
                ...
                function (data) {
                  if (data == "success") {
                    setTimeout(function () {
                      $("#statusDeposit2").fadeOut(500, function () {
                        $("#statusDeposit2")
                          .removeClass(
                            "alert alert-warning text-center glyphicon glyphicon-alert"
                          )
  ...
  $(document).on("click", ".kirim", function () {
    ...
    if (tulissubject != "") {
      if (memolength >= 10) {
        $.post("data2.php", $(".tulis_form").serialize(), function (data) {
          if (data == "success") {
  ...
  $(document).on("click", ".balas", function () {
    ...
    if (memolength >= 10) {
      $.post("data2.php", $(".balas_form").serialize(), function (data) {
        if (data == "success") {
  ...
  $(document).on("click", "#change_password", function () {
    ...
    $.post("desktop/data.php", $(".pass_form").serialize(), function (data) {
      setTimeout(function () {
        if (data == "passEmpty") {
        ...
        if (data == "passError") {
          $("#statusPassword").fadeOut(500, function () {
            $("#statusPassword")
              ...
              .addClass("alert alert-warning text-center")
              .text("Password Baru tidak diijinkan, silakan ganti!")
              .fadeIn(1000);
          });
        }
        ...
        if (data == "success") {
          $("#statusPassword").fadeOut(500, function () {
            $("#statusPassword")
              ...
              .addClass("alert alert-success text-center")
              .text("Terima kasih, Password berhasil diganti!")
              .fadeIn(1000);
          });
  ...
  $(document).on("click", "#addrek", function () {
    ...
    $.post("desktop/data.php", $(".addrek_form").serialize(), function (data) {
      if (data == "success") {
        ...
        alert(
          "Melebihi Max per Bank, Hubungi customer service untuk penambahan"
        );
        getRekening();
      } else {
  ...
  $(document).on("click", "#updaterek", function () {
    ...
    $.post("desktop/data.php", $(".updaterek_form").serialize(), function (data) {
      if (data == "success") {
                  ...
                  setTimeout(function () {
                    $("#statusWithdraw").fadeOut(500, function () {
                      $("#statusWithdraw")
                        ...
                        .addClass("alert alert-success text-center")
                        .text("Terima kasih, Withdraw berhasil disubmit!")
                        .fadeIn(1000);
                    });
                  }, 1000);
                } else if (data == "error") {
                  ...
                  setTimeout(function () {
                    $("#statusWithdraw").fadeOut(500, function () {
                      $("#statusWithdraw")
                        ...
                        .addClass("alert alert-danger text-center")
                        .text("Maaf Sedang ada Pending Withdraw")
                        .fadeIn(1000);
                    });
                  }, 1000);
                } else if (data == "receh") {
                  setTimeout(function () {
                    $("#statusWithdraw").fadeOut(500, function () {
                      $("#statusWithdraw")
                        ...
                        .addClass("alert alert-danger text-center")
                        .text(
                          "Maaf Withdraw harus kelipatan " + ribuan(kelipatan)
                        )
                        .fadeIn(1000);
                    });
                  }, 1000);
