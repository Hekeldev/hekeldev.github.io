...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath($_SERVER['SCRIPT_FILENAME'])) {
	...
	if ($aksi == 'daftar') {
		...
		// Memeriksa apakah bidang username dan sandi telah diisi
		if (empty($username)) {
			echo "userWrong";
			exit();
		}

		// Memeriksa apakah bidang username di isi dengan format yang benar
		...
		if (empty($_POST['sandi1']) || empty($_POST['sandi2'])) {
			echo "emptySandi";
			exit();
		}

		if (strlen($_POST['sandi1']) < 6) {
			echo "passWrong";
			exit();
		}

		if (strlen($_POST['sandi2']) < 6) {
			echo "shortSandiKonfirmasi";
			exit();
		}
			...
			if ($cek_email['jumlah'] > 0) {
				...
				if ($sandi !== $sandi2) {
					...
					if ($cek_kontak['jumlah'] > 0) {
						...
						// Memeriksa Nomor Handphone
						foreach ($mysqli->user_cek_rek($rek) as $cek_rek);
						if ($cek_rek['jumlah'] > 0) {
							echo 'rekExist';
						} else {
							// Validasi Berhasil->Insert dan Login
							$mysqli->daftar($username, $nama, $email, $sandi, $kontak, $nama_bank, $rek, $tanggal_daftar, $ip_daftar);
