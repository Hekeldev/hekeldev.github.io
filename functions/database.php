...
class databases
{
	...
	//User-General
	public function daftar($username, $nama, $email, $sandi, $kontak, $nama_bank, $rek, $tanggal_daftar, $ip_daftar)
	{
		$id = md5(date('Y-m-d H:i:s'));
		$this->conn->query("INSERT INTO user(username,nama,email,kontak,nama_bank,rek,password,tanggal_daftar,ip_daftar) values('$username','$nama','$email','$kontak','$nama_bank','$rek', '$sandi', '$tanggal_daftar', '$ip_daftar')");
	...
	public function user_cek_rek($rek)
	{
		$hasil = $this->conn->query("SELECT count(*)as jumlah from user where rek='$rek'");
		while ($d = mysqli_fetch_array($hasil)) {
			$result[] = $d;
		}
		return $result;
	...
	public function konfirmasi_pembayaran($invoice, $sesi_id, $bank, $rekening, $nama, $date, $gambar)
	{
		...
		if (in_array($file_extension, $valid_extension) && $ukuran < 2444070) {
			...
			echo "<script>alert('Deposit berhasil terkirim!')
	...
	public function konfirmasi_topup($id, $sesi_id, $bank, $rekening, $nama, $date)
	{
		...
		$angka_acak     = md5(rand(1, 9999999));
		$nama_gambar_baru = $angka_acak . '-' . $filename;
		$target_file = '../admin/upload/bukti_tf/' . $nama_gambar_baru;
		$file_extension = pathinfo($target_file, PATHINFO_EXTENSION);
		$file_extension = strtolower($file_extension);
		$valid_extension = array("png", "jpeg", "jpg");
		$ukuran	= $_FILES['gambar']['size'];
		...
		if (in_array($file_extension, $valid_extension) && $ukuran < 2444070) {
			...
			$this->conn->query("INSERT INTO saldo_validasi(id_user,nama_pengirim,bank_pengirim,rekening_pengirim,id_saldo,date,bukti)values('$sesi_id','$nama','$bank','$rekening','$id','$date','$nama_gambar_baru')");
			...
			if ($cek > 0) {
				if ($status == 0) {
					$this->conn->query("UPDATE saldo set status=9, remark='Upload bukti terlebih dahulu' where id='$id'");
				}
			}
	...
	public function saldo_akun($sesi_id)
	{
		...
		sum(case WHEN saldo_flow='in' and status=1 then nominal else null end) as masuk,
		sum(case WHEN saldo_flow='out' and status=1 then nominal else null end) as keluar
	...
	{
		$this->conn->query("INSERT INTO saldo(id_user,nominal,saldo_flow,remark,status,date) VALUES('$sesi_id', '$nominal', 'in', 'Pending', '0', '$date')");
		$last_id = $this->conn->insert_id;
	...
	{
		$this->conn->query("INSERT INTO saldo(id_user,nominal,saldo_flow,remark,status,date) VALUES('$sesi_id', '$harga', 'out', 'Pembayaran invoice #$id_invoice', '1', '$date')");
	}
